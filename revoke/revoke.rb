...
resource = "opencorn-" + Base64.encode64(OpenSSL::Random.random_bytes(9)).chomp.downcase
...
read_and_check_hashcash

puts "Thanks, now please paste your revocation blob, followed by an empty line."
revo_blob = ""
...
g.ls_files.keys.each do |key_file|
	puts "running openssl rsautl -verify -in #{signature_file.path} -inkey #{OpenCorn::Config['ACCEPTED_REPO']}/#{key_file} -keyform DER -pubin"
	sig_result = `openssl rsautl -verify -in #{signature_file.path} -inkey #{OpenCorn::Config['ACCEPTED_REPO']}/#{key_file} -keyform DER -pubin`[-32,32]
	next if ($? != 0)
...
if ! key_file_to_revoke then
	STDERR.puts "Sorry, no file to revoke found (maybe your revocation blob is wrong?"
	exit 3
end
...
File.copy(OpenCorn::Config['ACCEPTED_REPO'] + "/#{key_file_to_revoke}", OpenCorn::Config['REVOCATION_REPO'])
